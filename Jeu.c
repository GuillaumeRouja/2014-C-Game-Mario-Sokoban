...
void jouer(SDL_Surface* ecran)
{
    ...
    marioActuel = mario[BAS]; // Mario sera dirigé vers le bas au départ
     ...
     if (choixmenu)  // Variable booléenne globale me permettant de sélectionner le mode entrainement (valeur 1) ou le mode histoire (valeur 0)
     {
    ...
    while (continuer)
    {
            ...
            case SDL_KEYDOWN:
                switch(event.key.keysym.sym)
                {
                    case SDLK_ESCAPE:
                        continuer = 0;
                        break;
                    case SDLK_SPACE: // Réiitialisation du niveau
                        if (choixmenu)
                            {if (!chargerNiveau(carte))
                                exit(EXIT_FAILURE);}
                        else
                        ...
                        for (i = 0 ; i < NB_BLOCS_LARGEUR ; i++)  // Recherche de la position de Mario au départ
                            {
                            for (j = 0 ; j < NB_BLOCS_HAUTEUR ; j++)
                                {
                                if (carte[i][j] == MARIO) // Si Mario se trouve à cette position sur la carte
        ...
        // Si on n'a trouvé aucun objectif sur la carte, c'est qu'on a gagné on affiche alors l'écran bravo
        if (!objectifsRestants)
        {
            if (choixmenu==1)  // Arrêt du jeu et retour au menu en cas de victoire en mode jeu
                {bravojeu(ecran);
                continuer=0;}
            else
            {
            numeroNiveau=numeroNiveau+1;
            if (numeroNiveau>10)  // Arrêt du jeu et retour au menu en cas de victoire en mode histoire
                {bravohistoire(ecran);
                continuer=0;}
            if (!chargerNiveauxHistoire(carte,numeroNiveau))
            exit(EXIT_FAILURE);
